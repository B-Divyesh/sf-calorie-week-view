...
describe('static deployment routing', () => {
  ...
  it('keeps the installed app and offline cache on the current release', () => {
    ...
    expect(packageManifest.version).toBe('1.0.2');
    expect(webManifest.start_url).toBe('/app?v=1.0.2');
    expect(appSource).toContain("const BUILD_ID = '1.0.2'");
    expect(serviceWorker).toContain("const CACHE_NAME = 'calorie-week-view-v1.0.4'");
  });
});
